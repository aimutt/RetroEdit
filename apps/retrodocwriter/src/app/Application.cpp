...
void Application::MoveCursorRight()
{
    ...
    {
        ++m_cursor.column;
        // UTF-8: skip over remaining continuation bytes of the codepoint
        // we just crossed.
        const std::string& line = m_document->Buffer().Line(m_cursor.row);
...
void Application::Render()
{
    ...
    if (m_promptMode == PromptMode::None)
    {
        ...
        // the offending chars. (The cell-grid DrawEditorArea also tints
        // them but its output is painted over by m_wysiwyg->Draw.)
        if (m_highlightMisspelled)
        {
